      ...
      use hru_module, only : hru_db, ihru, sol_plt_ini, sol_plt_ini_cs, snodb, gldb
      ...
      do ihru = 1, sp_ob%hru
        ...
        ! SWAT+GL DB
        !do isgl = 1, db_mx%gl
        !    if (hru_db(i)%dbsc%gl == gldb(isgl)%name) then
        !       hru_db(i)%dbs%gl = isgl
        !    exit
        !    end if
        !end do         
        ! if (hru_db(i)%dbs%hyd == 0) write (9001,*) hru_db(i)%dbgl%hyd, "not found (glacier_hrus.gl)"
