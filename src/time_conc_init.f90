     ...
     ! compute weighted Mannings n for each subbasin
      ...
      do ihru = 1, sp_ob%hru
        ...
        !! Timo Modified Spatially Distributed Surlag
        if (bsn_cc%surlag_sd==0) then !! Basin Scale Surlag
            if (time%step > 1) then
        ...
        elseif (bsn_cc%surlag_sd==1) then!! HRU Scale Surlag - Timo Modified 
            if (time%step > 1) then
              brt(ihru) = 1.-Exp(-hru(ihru)%hyd%surlag / (tconc(ihru) /               &
                  (time%dtm / 60.)))
            else
              brt(ihru) = 1. - Exp(-hru(ihru)%hyd%surlag / tconc(ihru))
            endif
