...
!!    hydrologic cycle
      ...
      use hru_module, only : hru, ihru, tillage_switch,                                           &
         ...
         wetqsalt, wtspsalt,gwupsalt, glacc, glmlt, glsubl, hru_gl_obj, gwe,                      &
      ...
      if (bsn_cc%lapse == 1 .or. bsn_cc%lapse == 2) then
	 ...
	 !! compute reduction in pollutants due to in field grass waterway
      ...
      ! output_waterbal
        ...
        hwb_d(j)%glmlt = glmlt         ! SWAT+GL
        hwb_d(j)%glacc = glacc         ! SWAT+GL
        !hwb_d(j)%surq_gen = qday !! Original Qsurf Gen & Qsurf Cont flipped
        ...
        hwb_d(j)%glsubl = glsubl       ! SWAT+GL
        hwb_d(j)%wet_evap = hru(j)%water_evap 
        hwb_d(j)%wet_out = wet_outflow
        hwb_d(j)%wet_stor = wet(j)%flo / (10. * hru(j)%area_ha)
        !hwb_d(j)%surq_cont = surfq(j) !! Original Qsurf Gen & Qsurf Cont flipped
        hwb_d(j)%surq_cont = qday !! Timo Corrected
