...
!! Read in glacier information & glacierized area of all subbasins
       ...
       real:: va_c, va_gamma  ! V-A scaling parameters
       integer:: ierr, cnt, evo_model
        ...
        read(iu1,'(A)') titldum
        do i=1, cnt
            read(iu1, *, iostat=ierr) sm_model, ros_flag, gm_init, fgmlt_cor, pmix_flag, acc_mod, evo_model
        end do
        ...
        pfac = 0.
        pthr = 0.
        va_c = 0.037    ! Default V-A scaling constant (Ohmura et al. 1992)
        va_gamma = 1.36 ! Default V-A scaling exponent (Ohmura et al. 1992)
        rewind(iu1)
        read(iu1,'(A)') titldum
        do i=1, cnt
            read(iu1, *, iostat=ierr) tmix, pfac, pthr, va_c, va_gamma
        end do
