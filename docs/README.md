...
The file contains annual mass balance summaries. Annual refers to the glaciological year (October 1st to September 30th). Provided are information on the subbasin-wide net mass balance in m³ (*dM_m3*) where negative values indicating ablation and positive values accumulation (as w.eq.), the new water equivalent of ice for each ES in mm (*GWE_mm*) and the (glacier) area of each ES in km² (*Agl_km2*). Net mass balance change refers to the total change on the subbasin scale rather than the ES scale! Besides, specific annual, summer and winter mass balance results in meter w. eq. (*bn_m, bs_m, bw_m*). <br/>

The example in Figure 12 demonstrates the results of year 1962 with a negative mass balance (ablation) for 2 subbasins (subbasin 1 and 2) with 8 ES. Year 1960 refers to the starting year before warm up, indicated by 0 values for any change column. When an ES is ice-free it is indicated here by a 0 value for *GWE_mm*. The file contains the information of each year for all glacierized subbasins. Basin-wide information are not provided yet and must be aggregated by the user. You can just sum the *dM_m3* infos of each year of all subbasins for example to get total basin mass balance rates.  
