      ...
      type hydrology
           ...
           real :: surlag = 4         !!days          |4-20            |surface runoff lag time (days) - spatially distributed version
           real :: plaps = 0.5        !!mm/m          |0-500           |precipitation lapse rate (mm/m) - spatially distributed version
           real :: tlaps = 6.5        !!°C/km          |-10-10          |temperature lapse rate (°C/km)- spatially distributed version
      end type hydrology
      ...
      type snow_parameters
         ...
         real :: tfac_s = 2.5     !mm/deg C/d       |Temperature Melt Factor for HTI and ETI models Snow
         real :: rfac_s = 0.002   !mm*m²/d/W/deg C  |Radiation Factor Snow (HTI) 
         real :: srfac_s = 0.002  !mm*m²/d/W        |Radiation Factor Snow (ETI) 
         real :: pr_fac = 2       !mm/(mm rain)     |Wet-day rainfall melt Factor
         real :: f_exp = 0.5      !-                |Adjustment Factor for Exponential Snow Melt Model 
      end type snow_parameters
      type (snow_parameters), dimension (:), allocatable :: snodb
      
     ! Timo Glacier Pars
     type glacier_parameters
         character (len=40) :: name
         integer :: glhruids      !none             |actual/real HRU ID (e.g. The first glacier HRU might be HRU 32 within the model) 
         real :: glmtmp = 0.5   !deg C            |glacier melt temp 
         real :: glmfmx = 4.5     !mm/deg C/day     |Max melt rate for snow during year (June 21)
         real :: glmfmn = 4       !mm/deg C/day     |Min melt rate for snow during year (Dec 21)
         real :: f_accu= 0.1      !none             |snow pack temp lag factor (0-1)
         real :: f_frz = 0.03     !mm H20           |snow water content at full ground cover               
         !real :: tfac_s = 2.5     !mm/deg C/d       |Temperature Melt Factor for HTI and ETI models Snow
         real :: tfac_i = 2.5     !mm/deg C/d       |Temperature Melt Factor for HTI and ETI models Ice
         !real :: rfac_s = 0.002   !mm*m²/d/W/deg C  |Radiation Factor Snow (HTI) 
         real :: rfac_i = 0.002   !mm*m²/d/W/deg C  |Radiation Factor Ice (HTI)
         !real :: srfac_s = 0.002  !mm*m²/d/W        |Radiation Factor Snow (ETI) 
         real :: srfac_i = 0.002  !mm*m²/d/W        |Radiation Factor Ice (ETI) 
         !real :: pr_fac = 2       !mm/(mm rain)     |Wet-day rainfall melt Factor
         real :: gl_lag = 0.5     !-                |Glacier Temperature Lag Factor
         !real :: f_exp = 0.5      !-                |Adjustment Factor for Exponential Snow Melt Model 
     end type glacier_parameters     
     type (glacier_parameters), dimension (:), allocatable :: gldb
     ...
     type (glacier_mass_balance), dimension (:), allocatable :: glmb_a
        ...
        ! Glacier HRU 
        real, dimension (:), allocatable :: srfac_s    
        real, dimension (:), allocatable :: srfac_i 
        real, dimension (:), allocatable :: rfac_s    
        real, dimension (:), allocatable :: rfac_i    
        real, dimension (:), allocatable :: pr_fac
        real, dimension (:), allocatable :: tfac_s
        real, dimension (:), allocatable :: tfac_i    
        real, dimension (:), allocatable :: gl_lag   
        real, dimension (:), allocatable :: f_exp     
        real, dimension (:), allocatable :: glmtmp     !deg C         |glacier melt temp 
        real, dimension (:), allocatable :: glmfmx    !mm/deg C/day  |Max melt rate for snow during year (June 21)
        real, dimension (:), allocatable :: glmfmn   !mm/deg C/day  |Min melt rate for snow during year (Dec 21)
        real, dimension (:), allocatable :: f_accu        !none          |snow pack temp lag factor (0-1)
        real, dimension (:), allocatable :: f_frz     !mm H20        |snow water content at full ground cover
        integer, dimension(:), allocatable:: glhruids
        integer, dimension(:), allocatable:: hrus_sub_assign ! Subbasin ID in which HRU is located
        real, dimension(:), allocatable:: albedo_dd ! Degree Days Accumulation since Snowfall (per HRU) for better albedo estimates

        ! Glacier ES Infos from swat_gles.gl File
        integer, dimension(:), allocatable:: glsubs ! Glacier Subbasin ID
        integer, dimension(:), allocatable:: esids  ! ES ID
        !real, dimension(:), allocatable:: es_mx_elev ! Max ES Elev in that subbasin
        real, dimension(:), allocatable:: e_norm ! Normalied Elevation for dh approach
        real, dimension(:), allocatable:: dh     ! Normalied Elevation for dh approach
        real, dimension(:), allocatable:: es_glw ! mm  | Glacier Water Equivalent of ES
        real, dimension(:), allocatable:: es_gla  ! km2 | Glacier Area of ES
        real, dimension(:), allocatable:: es_gla_real  ! km2 | Glacier Area of ES after actual HRU calculation
        real, dimension(:), allocatable:: es_glv  ! km3 | Glacier Volume of ES
        real, dimension(:), allocatable:: es_glfr ! -   | Area Fraction of ES to total Subbasin Glacier Area 
        real, dimension(:), allocatable:: es_elup ! m   | ES Upper Elevation Boundary
        real:: esdist                             ! m   | Spacing between ES 
        ! Subbasin Glacier Infos
        integer, dimension(:), allocatable:: subs_gl  ! Unique Glacier Subbasin IDs
        integer, dimension(:), allocatable:: tot_glhru_sub  ! Total # of Glaciated HRUs in glaciated Subbasins
        real, dimension(:), allocatable:: gla_sub     ! km2  | Subbasin Glacier Area
        real, dimension(:), allocatable:: gla_sub_real! km2  | Subbasin Glacier Area based on actual HRU Areas 
        real, dimension(:), allocatable:: glw_sub     ! mm  | Subbasin Glacier Water Equivalent
        real, dimension(:), allocatable:: glw_sub_real     ! mm  | Subbasin Glacier Water Equivalent based on actual HRU Areas 
        real, dimension(:), allocatable:: glww_sub    ! mm  | Subbasin Weighted Water Equivalent
        real, dimension(:), allocatable:: glww_sub_real    ! mm  | Subbasin Weighted Water Equivalent based on actual HRU Areas 
        real, dimension(:), allocatable:: glv_sub     ! km3  | Subbasin Glacier Volume (Ice)
        real, dimension(:), allocatable:: glv_sub_real     ! km3  | Subbasin Glacier Volume (Ice) based on actual HRU Areas 
        real, dimension(:), allocatable:: glfr_sub_real ! -  | Subbasin Glacier Area Fraction w.r.t.Total Subbasin Area
        !! HRU Glacier Infos 
        real, dimension(:), allocatable:: hru_gla     ! km2  | HRU Glacier Area
        real, dimension(:), allocatable:: hru_glw     ! mm  | HRU Glacier Water Equivalent
        real, dimension(:), allocatable:: hru_glww    ! mm  | HRU Weighted Water Equivalent
        real, dimension(:), allocatable:: hru_glww2    ! mm  | HRU Weighted Water Equivalent
        real, dimension(:), allocatable:: hru_fr_es    ! -  | HRU Fraction of ES
        integer, dimension(:), allocatable:: hru_gl   ! -  | Unique Glacier Subbasin ID
        integer, dimension(:), allocatable:: hru_es_id   ! -  | ES ID to which HRU belongs
        !!Total HRU Glacier Assignment 
        integer, dimension(:), allocatable:: hru_isgl ! Is the current HRU glacierized? 0 = no 1 = yes 
        integer, dimension(:), allocatable:: hru_index_map ! Index Position of Glacier HRUs 
        !! Glacier Mass Balance Variables 
        real:: glmlt     ! mm  | Glacier Melt of HRU
        real:: glacc     ! mm  | Glacier Accumulation of HRU
        real:: glsubl    ! mm  | Glacier Sublimation of HRU
        real:: gwe       ! m   ! Glacier Water Equivalent of HRU
        real:: glsub_id 
        ...
        type es_db
            ...
            real :: es_mx_elev
            real :: es_mn_elev
            integer,dimension(:), allocatable:: es_id  
            real,dimension(:), allocatable::  es_glww       
            real,dimension(:), allocatable::  es_glw 
            real,dimension(:), allocatable::  es_glw_init       
            real,dimension(:), allocatable::  es_gla
            real,dimension(:), allocatable::  es_gla_scale
            real,dimension(:), allocatable::  es_gla_init     
            real,dimension(:), allocatable::  es_glv     
            real,dimension(:), allocatable::  es_glfr      
            real,dimension(:), allocatable::  es_elup       
         end type es_db
         type (es_db), dimension (:), allocatable :: es_obj, es_obj_real
